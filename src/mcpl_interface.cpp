...
// RETINA feature
#ifdef OPENMC_MCPL
void write_mcpl_retina_bank(mcpl_outfile_t file_id,
  gsl::span<RetinaSite> retina_bank, const vector<int64_t>& bank_index)
{
  int64_t dims_size = settings::n_particles;
  int64_t count_size = simulation::work_per_rank;
  ...
  if (mpi::master) {
    ...
    // Save retina bank sites since the array is overwritten below
#ifdef OPENMC_MPI
    vector<RetinaSite> temp_source {retina_bank.begin(), retina_bank.end()};
#endif
    ...
    for (int i = 0; i < mpi::n_procs; ++i) {
      ...
      if (i > 0)
        MPI_Recv(retina_bank.data(), count[0], mpi::retina_site, i, i,
          mpi::intracomm, MPI_STATUS_IGNORE);
#endif
      // now write the retina_bank data again.
      
      int ii = 0;

      for (const auto& site : retina_bank) {
      //Binary blob should be added before the mcpl_add_particle function 
      // TO ADD UNIV NUVLIDE MATERIAL MATERIAL
      std::ostringstream custom_data_stream;
      custom_data_stream << " dE : "<< site.dE << " ; event_mt : "<< site.event_mt 
      << " ; delayed_group : " << site.delayed_group << " ; cell_id : " << site.cell_id <<
    " ; nuclide_id : " << site.nuclide_id << " ; mat_id : " << site.mat_id  <<" ; univ_id : " <<
    site.univ_id << " ; parent_id : " << site.parent_id << " ; progeny_id : " << site.progeny_id;
      ...
      mcpl_hdr_add_data(file_id, CK.c_str(), custom_data_str.size(), custom_data_str.c_str());
      ii++;
      }
...
#ifdef OPENMC_MPI
    // Restore state of retina bank
    std::copy(temp_source.begin(), temp_source.end(), retina_bank.begin());
#endif
  } else {
#ifdef OPENMC_MPI
...
void write_mcpl_retina_point(const char* filename,
  gsl::span<RetinaSite> retina_bank, const vector<int64_t>& bank_index)
{
  std::string filename_(filename);
  const auto extension = get_file_extension(filename_);
  if (extension == "") {
    ...
    warning("write_mcpl_retina_point was passed a file extension differing "
            "from .mcpl, but an mcpl file will be written.");
  }
  ...
  write_mcpl_retina_bank(file_id, retina_bank, bank_index);
